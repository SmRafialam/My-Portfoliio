    ...
    <footer className="relative border-t border-white/10 bg-surface/60">
      <div className="container-page py-12">
        <div className="flex flex-col items-center gap-8 text-center">
            ...
            {socials.map((s) => {
              ...
              return (
                <a
                  ...
                  rel="noreferrer"
                  aria-label={s.name}
                  className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-slate-300 transition-all hover:-translate-y-1 hover:border-accent/50 hover:text-white"
                >
                  <Icon size={16} />
                </a>
              )
          ...
          <div className="flex w-full flex-col items-center gap-4 border-t border-white/5 pt-6 sm:flex-row sm:justify-between">
            <p className="text-sm text-slate-500">
              © {new Date().getFullYear()} {profile.name}. Built with React & Framer Motion.
            </p>
            <a
              href="#home"
              className="flex items-center gap-2 text-sm text-slate-400 transition-colors hover:text-accent2"
            >
